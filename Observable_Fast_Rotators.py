...
expect = (('Horizons> ', 'PAGE OFF\n'),
          ('Horizons> ', 'ROTPER<3.5, ROTPER>0.0, ADIST<2.5, QR>0.9\n'),
          (' ] :', 'y\n'))
for i in expect:
    jpl.read_until(i[0])
    jpl.write(i[1])
# At this point a list of matching asteroids should appear
fast_rotators_raw = jpl.read_until(' <cr>:')
fast_rotators_lines = fast_rotators_raw.split('\n')
...
# Parse the lines
num_rot = 0
for line in fast_rotators_lines:
...
feder.date = '2016/11/04 12:00'  # UT
...
'''print(feder.next_setting(sun))
print(feder.next_rising(sun))
print(ephem.hour)
print(start_of_Obs)
print(feder.date)'''
...
for i in range(num_rot):
    ...
    #lines = fast_rot_cand.get_ephemerides(730)  # Get information for UND observatory code
